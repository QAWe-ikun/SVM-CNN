...
1. **首次运行**会自动下载 CIFAR-10 数据集（约 170MB）
2. **训练 CNN 需要 GPU** 以获得合理速度（CPU 也可但较慢）
3. **内存需求**：建议至少 4GB RAM
4. **磁盘空间**：约 500MB（数据集 + 模型 + 结果）

## 🔍 常见问题

### Q: 如何修改 SVM 的核函数？

编辑 `src/svm_classifier.py` 或创建实例时指定：
```python
classifier = SVMSobelClassifier(kernel='linear')  # 或 'poly', 'sigmoid'
```
...
```python
# SVM
from svm_classifier import SVMSobelClassifier
svm = SVMSobelClassifier()
svm.load_model("models/svm_sobel_model.pkl")

# CNN
from cnn_classifier import CNNClassifier
cnn = CNNClassifier()
cnn.load_model("models/cnn_vgg_model.pth")
```
